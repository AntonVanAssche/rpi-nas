...
echo -e "[i] ${cyan}Installing prometheus. ${reset}"
	...
	sudo cp -r rpi-nas/config_files/prometheus.yml prometheus/
...
echo -e "[i] ${cyan}Installing node exporter. ${reset}"
	...
	mv node_exporter-$VERSION_NODE_EPORTER.linux-armv7/ node_exporter/
	...
	sudo cp -rf rpi-nas/config_files/prometheus.yml /home/pi/prometheus/
